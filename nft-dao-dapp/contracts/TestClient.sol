...
contract ATestnetConsumer is ChainlinkClient, ConfirmedOwner {
  ...
  uint256 public currentNFTValue;
  ...
  event RequestNFTValueFulfilled(
    bytes32 indexed requestId,
  ...
  function requestNFTValue(address _oracle, string memory _jobId,string memory nft_contract, string memory nft_id)
    public
    onlyOwner
  {
    Chainlink.Request memory req = buildChainlinkRequest(stringToBytes32(_jobId), address(this), this.fulfillNFTValue.selector);
    req.add("nftContract", nft_contract);
    req.add("nftId", nft_id);
  ...
  function fulfillNFTValue(bytes32 _requestId, uint256 _nftvalue)
    public
    recordChainlinkFulfillment(_requestId)
  {
    emit RequestNFTValueFulfilled(_requestId, _nftvalue);
    currentNFTValue = _nftvalue;
  }
